...
# 5. STATUS BAR
status_text_color = styles.get_best_text_color(color)
st.markdown(f"""
<div style="margin-bottom: 20px; margin-top: 5px;">
    ...
    <div class="gov-pill" role="status" style="background: linear-gradient(135deg, {color}, {color}88); border: 1px solid {color}; color: {status_text_color};">{status}</div>
...
if full_data is not None and closes is not None:
    ...
    with tab2:
        ...
        with col1:
            status_text_color = styles.get_best_text_color(color)
            st.markdown(f'<div class="gov-pill" role="status" style="background: linear-gradient(135deg, {color}, {color}88); border: 1px solid {color}; color: {status_text_color};">{status}</div>', unsafe_allow_html=True)
