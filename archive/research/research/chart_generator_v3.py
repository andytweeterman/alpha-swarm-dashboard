...
# INCREASED VOLATILITY for better visual separation
for i in range(1, n_history):
    change = np.random.normal(0, 35) # Increased from 25
...
for i in range(15): # Increased density slightly
...
# --- NEW: DYNAMIC CAMERA ZOOM ---
# We calculate the absolute lowest and highest points in the visible data
data_min = min(np.min(lows), np.min(lower_bound))
data_max = max(np.max(highs), np.max(upper_bound))
price_range = data_max - data_min
buffer = price_range * 0.15 # Add 15% breathing room top/bottom
...
# ==========================================
ax.set_title("ALPHA SWARM: S&P 500 PROJECTION (Module V3 - Zoom Fix)", fontsize=16, color='white', fontweight='bold', pad=20)
ax.grid(True, color='gray', alpha=0.2)
ax.set_facecolor('black')
fig.patch.set_facecolor('black')
plt.tight_layout()

# Force save to script directory
