...
# ==========================================
# 1. FORCE FILE PATHS (The Fix)
# ==========================================
# This gets the folder where THIS script is currently sitting
script_dir = os.path.dirname(os.path.abspath(__file__))

# We force the output files to live in that same folder
txt_path = os.path.join(script_dir, "Bad_Date_Audit_Report.txt")
csv_path = os.path.join(script_dir, "audit_data_full.csv")

# Redirect output to both screen AND file
class Logger(object):
    def __init__(self):
        self.terminal = sys.stdout
        self.log = open(txt_path, "w")  # Use the absolute path
...
print("--- [ALPHA SWARM: BAD DATE AUDIT v3 (FIXED PATHS)] ---")
print(f"Saving reports to: {script_dir}")
...
def get_level(row):
    ...
    elif vol_bad: return "VOLATILE (Level 4)" 
