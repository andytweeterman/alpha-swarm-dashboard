...
def optimized():
    ...
    sub_l_cone = l_cone[l_cone.index >= start_filter]
    sub_u_cone = u_cone[u_cone.index >= start_filter]

    start_date = c_data.index[0]

    sub_ppo = ppo[ppo.index >= start_date]
    sub_sig = sig[sig.index >= start_date]
    sub_hist = hist[hist.index >= start_date]

    colors = ['#00ff00' if v >= 0 else '#ff0000' for v in sub_hist]

def optimized_mask():
    mask = full_data.index >= start_filter
    c_data = full_data.loc[mask]
